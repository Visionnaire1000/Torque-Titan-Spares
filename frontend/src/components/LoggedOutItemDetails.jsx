...
const LoggedOutItemDetails = () => {
  ...
  const fetchItemAndReviews = async () => {
    ...
    try {
      const itemRes = await fetch(`${config.API_BASE_URL}/spareparts/${id}`);
      if (!itemRes.ok) throw new Error("Item not found");

      const itemData = await itemRes.json();

      const reviewsRes = await fetch(`${config.API_BASE_URL}/reviews/${id}`);
      const reviewData = reviewsRes.ok ? await reviewsRes.json() : [];

      const reviewsFormatted = Array.isArray(reviewData)
        ? reviewData.map((r) => ({
            ...r,
    ...
    reviews.map((r) => (
      <div key={r.id} className="review-card">
        <div className="review-header">
          <span className="review-user">{r.display_name}</span>
